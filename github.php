	...
	public function __construct()
	{
		...
		self::$config = array(
			...
			'unparsed_responses'	=> array(
				...
				'issues'	=> '{repo}: 3{user} opened an issue at ({date}): {title} - {url}',
				'pulls'		=> '{repo}: 3{user} has requested to merge 12{head_label} into 7{base_label}: {title} - {url}',
				// unrequested responses, ie event based
				'commit'	=> '{repo}: commit added by 3{user} on ({date}): {title} - {url}',
				'issue'		=> '{repo}: #{id} opened by 3{user} on ({date}) ({comments}) (12{state}): {title} - {url}',
	...
	public function main( $xbot, $ircdata )
	{
		...
		if ( $ircdata->type == 'privmsg' && $ircdata->message[0] == ':' )
		{
			...
			if ( strcasecmp( $messages[0], 'track' ) == 0 )
			{
				...
				mysql_query( "INSERT INTO `".self::$config['mysql']['table_c']."` (`repo`, `chan`) VALUES('".$repo."', '".$info."')" );
			...
			foreach ( self::$config['recursive_api_calls'] as $id => $call )
			{
				...
				if ( $id == ( 'issues' || 'pulls' ) )
				{
					...
					foreach ( $data[$id] as $i => $rep )
					{
						...
						$git_reps_q = mysql_query( "SELECT `repo`, `info_id` FROM `".self::$config['mysql']['table_i']."` WHERE `info_id` = '".$rep['number']."' AND `type` = '".$id."' AND `repo` = '".$git_chans['repo']."' AND `timestamp` = '".$timestamp."'" );
						...
						$changed++;
						mysql_query( "INSERT INTO `".self::$config['mysql']['table_i']."` (`type`, `repo`, `info_id`, `timestamp`) VALUES('".$id."', '".$git_chans['repo']."', '".$rep['number']."', '".$timestamp."')" );
					...
					if ( $changed != 0 )
					{
						$encoded_data = mysql_real_escape_string( json_encode( $data ) );
						mysql_query( "INSERT INTO `".self::$config['mysql']['table_p']."` (`timestamp`, `payload`, `read`, `type`) VALUES('".time()."', '".$encoded_data."', '0', '".$id."')" );
					}
	...
	*/
	static public function parse_issues( $xbot, $payload )
	{
		foreach ( $payload['issues'] as $id => $issue )
		{
			...
			$title = ( strlen( $issue['title'] ) > 150 ) ? substr( $issue['title'], 0, 150 ) : $issue['title'];
			$msg = str_replace( '{title}', $title, $msg );
		...
		// output info like so:
		// acorairc: n0valyfe opened an issue at (2011/07/07 18:53:09 -0700): Issue title is this lump of text etc - https://github.com/ircnode/acorairc/issues/33
	}
	...
	*/
	static public function parse_pulls( $xbot, $payload )
	{
		foreach ( $payload['pulls'] as $id => $pull )
		{
			...
			$title = ( strlen( $pull['title'] ) > 150 ) ? substr( $pull['title'], 0, 150 ) : $pull['title'];
			$msg = str_replace( '{title}', $title, $msg );
		...
		// output info like so:
		// acorairc: n0valyfe has requested to merge n0valyfe:bugfix into ircnode:acorairc: Issue title is this lump of text etc - https://github.com/technoweenie/faraday/pull/15
	}
